...
Model::Model(daxa::Device device, const std::string& path) : device{device}, path{path} {
    ...
    auto get_image_type = [&](const tinygltf::Model& model, usize image_index) -> Texture::TextureType {
        for(auto& material : model.materials) {
            if(material.pbrMetallicRoughness.baseColorTexture.index != -1) {
                ...
                if (image_index == diffuseTexture.source) {
                    return Texture::TextureType::SRGB;
                }
            }
        }

        return Texture::TextureType::UNORM;
    };
...

    {
        ...
        constexpr auto gltfOptions = fastgltf::Options::DontRequireValidAssetMember | fastgltf::Options::LoadGLBBuffers | fastgltf::Options::LoadExternalBuffers;
        fastgltf::GltfDataBuffer data;
        data.loadFromFile(_path);

        if (_path.extension() == ".gltf") {
            gltf = parser.loadGLTF(&data, _path.parent_path(), gltfOptions);
        } else if (_path.extension() == ".glb") {
            gltf = parser.loadBinaryGLTF(&data, _path.parent_path(), gltfOptions);
        }
        ...
        if (error != fastgltf::Error::None) {
            throw std::runtime_error("Failed to load glTF: " + fastgltf::to_underlying(error));
        }

        asset = gltf->getParsedAsset();
    }

    daxa::CommandList cmd_list = device.create_command_list({ .debug_name = "up load buffers" });

    std::vector<daxa::BufferId> buffers_id = {};

    auto& buffers = asset->buffers;
    buffers_id.reserve(buffers.size());
    ...
    for (auto& buffer : buffers) {
        switch (buffer.location) {
            ...
            case fastgltf::DataLocation::CustomBufferWithId: {
                throw std::runtime_error("fix this bullshit");
                break;
            }
