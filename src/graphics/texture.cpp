...
namespace dare {
    ...
    auto Texture::load_texture(daxa::Device device, u32 size_x, u32 size_y, unsigned char *data, Type type) -> std::pair<std::unique_ptr<Texture>, daxa::CommandList> {
        u32 mip_levels = static_cast<uint32_t>(std::floor(std::log2(std::max(size_x, size_y)))) + 1;
        ...
        return std::pair{std::move(tex), cmd_list};
    }
