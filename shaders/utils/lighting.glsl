...
f32vec3 calculate_directional_light(DirectionalLight light, f32vec3 frag_color, f32vec3 normal, f32vec3 frag_position, f32vec3 camera_position) {
    ...
    f32vec3 diffuse = frag_color * light.color * max(dot(normal, light_dir), 0) * light.intensity;
    return diffuse;
#endif
    ...
    f32 specular = max(dot(view_dir, reflect(-light_dir, normal)), 0);
    ...
    f32 diffuse = max(dot(normal, light_dir), 0);
    f32 specular = max(dot(view_dir, halfway_dir), 0);
    return frag_color * light.color * (diffuse + specular) * light.intensity;
#endif
...
#if defined(SETTINGS_SHADING_MODEL_LAMBERTIAN)
    f32vec3 diffuse = frag_color * light.color * max(dot(normal, light_dir), 0) * attenuation * light.intensity;
    return diffuse;
#endif
#if defined(SETTINGS_SHADING_MODEL_PHONG)
    f32vec3 view_dir = normalize(camera_position - frag_position);
...
#if defined(SETTINGS_SHADING_MODEL_LAMBERTIAN)
    f32vec3 diffuse = frag_color * light.color * max(dot(normal, light_dir), 0) * attenuation * light.intensity * intensity;
    return diffuse;
