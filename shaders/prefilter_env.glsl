...
vec3 prefilterEnvMap(vec3 R, float roughness) {
    ...
    float envMapDim = float(get_cube_map_size(push_constant.env_map, 0).s);
    for(uint i = 0u; i < num_samples; i++) {
        ...
        if(dotNL > 0.0) {
            ...
            color += get_cube_map_lod(push_constant.env_map, L, mipLevel).rgb * dotNL;
            totalWeight += dotNL;
...
void main() {
    ...
    out_color = vec4(prefilterEnvMap(N, push_constant.roughness), 1.0);
}
